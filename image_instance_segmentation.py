...
The Inception V2 pre-trained model checkpoint is: January 28 2018 Link to the model: http://download.tensorflow.org/models/object_detection/mask_rcnn_inception_v2_coco_2018_01_28.tar.gz
...
parser = argparse.ArgumentParser(description='Image instance segmentation using Mask-RCNN built on InceptionV2')
parser.add_argument('-i', '--image', help='path to an input image')
parser.add_argument('-v', '--video', help='path to an input video')
...
winName = 'Image Instance Segmentation'
cv.namedWindow(winName, cv.WINDOW_NORMAL)

outputFile = "webcam-result.avi"
if (args.image):
    # Open the image file
    if not os.path.isfile(args.image):
        print("Input image file ", args.image, " not found.")
        sys.exit(1)
    cap = cv.VideoCapture(args.image)
    outputFile = args.image[:-4]+'_result.jpg'
elif (args.video):
    # Open the video file
    if not os.path.isfile(args.video):
        print("Input video file ", args.video, " not found.")
        sys.exit(1)
    cap = cv.VideoCapture(args.video)
    outputFile = args.video[:-4]+'_result.avi'
else:
    # Webcam input
